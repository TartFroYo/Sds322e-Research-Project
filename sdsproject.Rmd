...
We found the mean, median, maximum, and minimum for the following variables if it was possible.
```{r sss}
...
# Visualizing
Create a plot graphing one variable.
```{r vis1}
...
crime_rate |> 
  ...
  labs(title = "U.S. State Distribution of All Violent Crime Rates ", y = "Frequency") + 
  scale_x_continuous(name = "All Violent Crime Rates", breaks =seq(0, 1245,150)) + 
  theme_bw()
...
Create a second plot graphing one variable.
```{r vis2}
...
rqs %>% 
  ...
  labs(title = "Distribution of Federal Funds to State Public Schools", 
       y = "Frequency") +
  scale_x_continuous(name = "U.S. Federal Funding to State Public Schools by Hundred Thousands") +
...
Create a plot graphing two variables.
```{r vis3}
...
rqs %>% 
  ggplot(aes(x = Year, y = Federal_Revenue)) + 
...
Create a second plot graphing two variables.
```{r vis4}
...
rqs %>% 
  ...
  scale_y_discrete(c("U.S. State Median Household Income ($)")) + 
  scale_x_discrete(name = "Years (1992-2016)") +  
  # theme(axis.text.x = element_text(angle=90)) + 
  theme_bw() +
#Connect each state's points with a line
...
Create a plot graphing three variables. We split the rsq dataset in half so that the data points on the scatterplots are more readily seen.
```{r vis5}
...
Create a second plot graphing three variables. We split the rsq dataset in half so that the data points on the scatterplots are more readily seen.
```{r vis7}
# find max murder rates
summary(rqs$All_Property_Burglary_Rates)
